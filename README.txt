...
* Improve  - Admin rebuilt on the shared Wbcom settings shell: Endpoints and FAQ tabs on shell field components, flat getting-started steps and roomier cards.
* Fix      - Undefined $endpoint PHP warning in the group menu template removed.
* Fix      - Avatar handling no longer strips other plugins' get_avatar filters.
* Fix      - Clean uninstall of plugin data, and the Style logout colours now apply.
* Dev      - Removed the WPPB loader (hooks register directly) and cleared dead code.
...
* Fix: Admin AJAX handler now enforces POST method instead of accepting any request method
* Fix: Endpoint page titles properly sanitized with sanitize_text_field() instead of stripslashes()
* Fix: Custom endpoint content output wrapped with wp_kses_post() for proper escaping
* Fix: Avatar output in menu template wrapped with wp_kses_post() for proper escaping
* Fix: Elementor preview detection uses $_GET instead of $_REQUEST
* Fix: Replaced time() with plugin version constant for proper asset cache busting
* New: Declared WooCommerce HPOS (High-Performance Order Storage) compatibility
* New: Added WC tested up to (9.8) and WC requires at least (6.0) headers
* New: Proper plugin data cleanup on uninstall (options, transients, rewrite rules)
* New: Preset free license key for automatic updates without manual activation
* Tested up to WordPress 6.9.1

= 1.6.2 =
* Fix: Resolved endpoints AJAX error on My Account page
* Fix: wcmp_get_endpoint_by() was using wrong settings key causing incorrect endpoint resolution
