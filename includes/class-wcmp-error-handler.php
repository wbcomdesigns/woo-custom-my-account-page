...
class WCMP_Error_Handler {
	 ...
	 */
	public static function safe_get_option( $option_name, $default = array() ) {
		try {
	...
	public static function safe_array_get( $array, $key, $default = null ) {
		if ( ! is_array( $array ) ) {
			self::log_error( 'Expected array but got ' . gettype( $array ), 'safe_array_get' );
			return $default;
		}
	...
	public static function init() {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			set_error_handler( array( __CLASS__, 'custom_error_handler' ), E_USER_ERROR | E_USER_WARNING );
		}
	}
	...
	public static function custom_error_handler( $errno, $errstr, $errfile, $errline ) {
		...
		if ( strpos( $errfile, 'woo-custom-my-account-page' ) === false ) {
			return false;
		}
