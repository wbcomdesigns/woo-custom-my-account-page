...
class WCMP_License {
    ...
    public function register_license_settings() {
        register_setting( 'wcmp_license', $this->license_key_option );
    }
    ...
    public function activate_license() {
        ...
        $response = wp_remote_post( WCMP_STORE_URL, array(
            'timeout' => 15,
            'body' => $api_params
        ) );
        ...
        if ( ! empty( $message ) ) {
            $redirect = add_query_arg( array( 'sl_activation' => 'false', 'message' => urlencode( $message ) ), admin_url( 'admin.php?page=wcmp-license' ) );
            wp_redirect( $redirect );
            exit();
        }

        update_option( $this->license_status_option, $license_data->license );
        wp_redirect( admin_url( 'admin.php?page=wcmp-license' ) );
        exit();
    }
    ...
    public function init_updater() {
        ...
        $edd_updater = new WCMP_Updater( WCMP_STORE_URL, WCMP_PLUGIN_FILE, array(
