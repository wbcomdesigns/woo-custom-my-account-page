...
class Woo_Custom_My_Account_Page_Public {
	...
	public function wccma_enqueue_scripts() {
		if ( is_account_page() ) {
			...
			wp_localize_script(
			$this->plugin_name, 'wccma_public_js_obj', array(
	...
	public function wccma_redirect_default_my_account_tabs() {
		global $woo_custom_my_account_page;
		if ( !is_wc_endpoint_url() && isset( $woo_custom_my_account_page->default_woo_tab ) && $woo_custom_my_account_page->default_woo_tab != '' ) {
			$endpoint = $woo_custom_my_account_page->default_woo_tab;
			if ( $endpoint != 'dashboard' ) {
				exit( wp_redirect( wc_get_account_endpoint_url( $endpoint ) ) );
			}
		}
	}

	function wccma_add_menu_style_footer() {
		global $woo_custom_my_account_page;
		if ( isset( $woo_custom_my_account_page->menu_style ) && $woo_custom_my_account_page->menu_style == 'tab' ) {
			?>
			<style>
				/*Tab*/
